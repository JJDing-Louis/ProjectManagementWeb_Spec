...
每位使用者登入後，只會看到自己有權限查看的專案與 Task Item。`Viewer` 除了自己的顯示名稱與電話號碼外，只能瀏覽資料，不能新增、修改或刪除業務資料。取得其他角色後，被指派者才可以修改自己 Task Item 的狀態與交付時間，也可以在 Task 詳情中留言。
...
- 登入使用者可修改自己的顯示名稱與電話號碼。顯示名稱 trim 後為 1–100 個字元；電話為選填，空白表示清除，非空值 trim 後最多 30 個字元且必須符合電話格式。帳號、Email、系統角色與啟用狀態不包含在個人資料更新 request。
...
2. 系統會透過 Gmail SMTP 寄出 E-mail 驗證信；完成驗證前，使用者只能以 `Viewer` 身分登入及瀏覽有權限查看的資料，但仍可維護自己的顯示名稱與電話號碼，不得修改業務資料
...
3. 最高管理者（`Admin`）
4. 瀏覽者（`Viewer`）：除自己的顯示名稱與電話號碼外，只能查看有權限存取的資料，不得新增、修改、刪除、留言、切換 Task 狀態或調整角色
